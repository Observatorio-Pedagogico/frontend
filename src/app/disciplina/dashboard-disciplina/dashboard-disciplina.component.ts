  ...
  montarGraficoNotasDisciplinas() {
    ...
    aplicarFiltro() {
      if (this.formFiltroAlunosReprovadosPorFalta.value !== '') {
        this.ignorarReprovadosPorFalta = this.formFiltroAlunosReprovadosPorFalta.value;
      } else {
        this.ignorarReprovadosPorFalta = 'false';
      }
      this.parametrosDashboards = `?periodoLetivo=${this.formFiltroPeriodo.value!.toString().replace(/[^[]]/gi,'')}&ignorarReprovadosPorFalta=${this.ignorarReprovadosPorFalta}`;
      this.montar();
    }
